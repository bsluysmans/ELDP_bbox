...
DATE: 22/03/2015
=============================================================
MIQP for ELDP: general problem adapted for sinus function
"""
...
while (value(instance.delta) > value(instance.eps) and count < MaxIter and max_m < length):
	...
	for i in instance.I:
		for j in xrange(1,instance.m[i]+1):
			instance.alpha[i,j] = instance.d[i]*(abs(sin(instance.e[i]*(instance.Xi[i,j+1]-instance.p_min[i])))-abs(sin(instance.e[i]*(instance.Xi[i,j]-instance.p_min[i]))))/(instance.Xi[i,j+1]-instance.Xi[i,j])
			instance.beta[i,j] = instance.d[i]*abs(sin(instance.e[i]*(instance.Xi[i,j]-instance.p_min[i])))-instance.alpha[i,j]*instance.Xi[i,j]
	...
	for i in instance.I:
		...
		for j in xrange(1,instance.m[i]+2):
			...
			if (abs(value(instance.p[i]) - value(instance.Xi[i,j])) <= value(instance.eps)):
				add = False
